...
/* macro to inform if board clock initialised or not*/
#define BOARD_CLOCK_INIT_DONE_BY_BOOTLOADER		0u
#define BOARD_CLOCK_INIT_NOTDONE_BY_BOOTLOADER	1u

#define BOARD_CLOCK_INIT_CHECK 					BOARD_CLOCK_INIT_NOTDONE_BY_BOOTLOADER
...
/* check if board clock is alread been initialised earlier bz Bootloader board files */	
#if (BOARD_CLOCK_INIT_CHECK == BOARD_CLOCK_INIT_NOTDONE_BY_BOOTLOADER)	
	boardEnableClocks();
	boardSetupClocks();
#endif
	/*test_leds();*/
